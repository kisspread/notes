...
![alt text](../../assets/images/ST01_image-3.webp)

## My Understanding

状态是有类型`Type`的，官方的定义如下：
...
只有`State`类型，才能配置选择行为`Selection Behavior`，也可以叫做选择子状态的模式。

### 默认状态模式`Try Select Children In Order`
...
### 中间态模式 `Try Enter`
既`Try Enter`, 这种选择行为，允许状态不去到叶子节点，而是停留在"中间态", 子状态被忽略 。

相比于默认模式一定要有一个可用的叶子状态，该模式用起来更加灵活。比如实际开发过程中，经常会遇到"暂时"没有适合的叶子状态，叶子状态还需要依赖外部Event来触发，这种情况下`Try Enter`模式就很有用。

我们可以让中间态的`Transition`的`OnEvent`模式来配置对外部Event的监听，让中间态来决定如何切换到相应的子状态。

#### Try Enter的大坑 之Tick陷阱（5.5.4 和 5.6.0 均能复现）
这种中间状态，通常是搭配 外部Event来切换状态；但有时候，并不想通过外部Event触发，而是该状态主动切换到具体的叶子状态。不过我在测试了之后，发现时不时存在状态无法跳出的问题。

这种行为的不一致性非常让人困惑。

两种行为的简单概括：
1. 如果叶子状态的第一个Task没有Tick行为，仅在`EnterState` 执行后立即FinishTask，那么可以正常触发该状态的`Transition`
2. 如果叶子状态Task有Tick行为，比如`DelayTask`，那么会陷入死循环，无法逃离该状态。

![alt text](../../assets/images/ST01_image-9.webp){width=60%}
图为出现无限循环的情况。

具体就是，中间态的`Transition`的配置了如`OnStateCompleted`的规则，切换到了具体叶子状态，这时如果子状态的Task存在Tick行为（有东西在`OnTick`里面执行），则会在该状态和子状态之间死循环！ 子状态无法根据自身的`Transition`来切换到其他状态， 因为子状态的“完成”会触发父状态的“完成”，导致`Transition`被父状态覆盖，而父状态（既这里的中间态）的转换规则又指向了该子状态，导致在这个叶子状态里面无限循环。

那为什么不直接用`Try Select Children In Order`这个默认模式？因为这个模式父子的Tick是一起执行的，不符合需求，我不想要任何一个具体的叶子状态执行。 非要用`Try Select Chidren In Order`也可以，就是需要一个什么都不干的空状态。

#### Try Enter的大坑 之 前置Tag阻断进入

带Reuired Event Tag的中间态，无法主动进入自身的子状态(UE 5.5.4)
![alt text](../../assets/images/ST01_image-5.webp){width=50%}
...
![alt text](../../assets/images/ST01_image-6.webp){width=50%}

一种Workaround是：
这种情况不使用嵌套状态，虽然表达力下降，但至少不会出错。
![alt text](../../assets/images/ST01_image-7.webp){width=50%}


### 仅分组模式`Group`
当状态被设置为`Group`类型时，就无法定义选择模式。所以默认状态下，Group模式无法切换到子状态，这和中间态模式很像。由于Group状态本身无法添加任何Task，所以如果他有`Transition`规则，就会立即触发，既可以手动配置`Transition`规则来选择子状态。
...
### StateTree 使用感悟
- 如果没有没有配置默认的`Transition`，隐式的规则是返回到Root，但会有个报错，会影响debug的效率，建议全部进行显式配置即使是返回到Root

- 子状态可以访问父状态的上下文，比如 父状态的**Parameters**，以及全部Tasks的**Output**等；同一状态里面的Tasks，排在后面的Task可以访问前面Task的**Output**。而兄弟状态之间，无法通过**Output**来传递数据，所以可以给他们放进一个父状态来共享部分作用域。
...
- 叶子状态可以理解成“**动作**”，因为通常都很具体，而父状态（中间态）才是真正的“**状态**”
  ![alt text](../../assets/images/ST01_image-4.webp)
...
- 父状态一定要养成判断`OnEnter`和`OnExit`进入类型的习惯。如果没判断，很容易造成不必要的调用，或者奇怪的逻辑错误。
  ```cpp
    ...
    enum class EStateTreeStateChangeType : uint8
    {
        ...
        /** The state became activated or deactivated. */ 叶子状态100%是这种类型
        Changed,
        
        /** The state is parent of new active state and sustained previous active state. */ 父状态有可能是这种“保持”类型，它已经不是第一次激活了，只是孩子们自己切换了状态，父状态也会调用相关的Enter/Exit
        Sustained,
    };
    ```
...
![alt text](../../assets/images/ST01_image-1.webp){width=30%}
如图，唯一可用的子状态配置了进入条件，既只能通过外部Event来触发。这种情况下，父状态的模式理论上应该使用`Try Enter`,也就是中间态，但这个模式会触发Bug：
![alt text](../../assets/images/ST01_image.webp){width=30%}
...
2. 添加一个什么都不做的状态，让父状态有可用的子状态，这样父状态的Task就能正常执行了。
   ![alt text](../../assets/images/ST01_image-2.webp){width=20%}
...
![alt text](../../assets/images/ST01_image-8.webp){width=50%}
