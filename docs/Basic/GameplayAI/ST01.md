...
#### Try Enter的大坑（5.5.4 和 5.6.0 均能复现）
...
那为什么不直接用`Try Select Children In Order`这个默认模式？因为这个模式父子的Tick是一起执行的，不符合需求，我不想要任何一个具体的叶子状态执行。 

这种行为的不一致性非常让人困惑。

两种行为的简单概括：
1. 如果子状态的第一个Task没有Tick行为，是在`EnterState` 里面执行一些逻辑后，直接FinishTask，那么可以成功触发该状态的`Transition`
2. 如果子状态的第一个Task有Tick行为，比如`DelayTask`，那么会死循环，无法逃离该状态。
