...
![alt text](../assets/images/zbuffer-image.webp)
...
- Lambertian Cosine Law 
  ![alt text](../assets/images/101_image.webp)
- Lambertian reflection 模型认为漫反射和视角方向无关
  ![alt text](../assets/images/101_image-2.webp)
- max(0,n·l) 是为了忽略负数
- I/r^2 能量守恒，辐射半径越远，球的表面积越大，根据面积公式可推理
  ![alt text](../assets/images/101_image-3.webp)
...
![alt text](../assets/images/101_image-39.webp)
...
![alt text](../assets/images/101_image-49.webp)
...
### 三角形的重心
 ...
 - 三角形的重心，相当于这个坐标系的"0点"，三个顶点的均值位置。
   ![alt text](../assets/images/101_image-8.webp)

 - 利用重心坐标系，就可以进行对各种基于顶点的属性，进行顶点插值
   ![alt text](../assets/images/101_image-9.webp)  
...
- 为了解决这个问题，需要在纹理和像素之间进行插值，就是纹理映射。
  ![alt text](../assets/images/101_image-10.webp)
- Nearest:
  最近邻插值，取最接近的纹素的颜色，会导致明显的锯齿
- Bilinear:
  双线性插值，取最接近的两个纹素的颜色，可以避免锯齿，但是会导致边缘模糊。（做了3次插值） 
  ![alt text](../assets/images/101_image-11.webp)
- Bicubic:
...
![alt text](../assets/images/101_image-12.webp)

![alt text](../assets/images/101_image-13.webp)
...
- Mipmap被设计成预计算后存起来，仅增加1/3的显存占用。每层Mipmap的尺寸都是上一层的一半，直到最后一层，尺寸为1x1。。极限求和是1/3。
  ![alt text](../assets/images/101_image-14.webp)
...
- 假如某个像素覆盖了很多纹素，这会在纹理上投影出一个大点P(覆盖了很多个纹素)，那么再取邻近像素的对应的纹素大点Q，（相邻可以是上下左右）获得两个点之间最大距离L，假如L是8,就代表信息量缩小了8倍，用log2(L) 就可以知道它要用第3层Mipmap来查询均值。
  ![alt text](../assets/images/101_image-15.webp)
...
- 但Spherical Environment Map 被处理成为 方形后，上下边缘部分有严重的拉伸。
  ![alt text](../assets/images/101_image-20.webp)
- 为了解决这个问题，需要用Cubemap来表示，就是在反射球套上一个正方体，这样就不会出现拉伸问题了。
  ![alt text](../assets/images/101_image-21.webp)  
...
- 法向计算：微分，获得向量，V(1,dp)的垂直变换就是 N(-dp,1)。3d同理。
  ![alt text](../assets/images/101_image-22.webp)

- Displacement Mapping，纹理置换，修改顶点来影响模型的最终展示。好处是比Bump Mapping更加真实。
  ![alt text](../assets/images/101_image-23.webp)
...
- 纹理映射，也是显示的表示，虽然看起来是函数，但值是预先记录在纹理里的，只是映射期间用了计算。（其实我觉得应该叫做半隐式的表示）
  ![alt text](../assets/images/101_image-24.webp)

- 皮克斯高级研究人员，宣称很讨厌Meshs，它太难了。
  ![alt text](../assets/images/101_image-25.webp)  

- 布尔运算，是非常常见的隐式表示：
  ![alt text](../assets/images/101_image-26.webp)

- SDF 对某些计算友好，比如混合（插值），还不占用磁盘空间，毕竟只是一个函数。图中对边界进行了插值。  
  ![alt text](../assets/images/101_image-27.webp)

- Level Set, fractal (没有细说)
  ![alt text](../assets/images/101_image-28.webp)  
  ![alt text](../assets/images/101_image-29.webp)

### wavefront OBJ 文件格式

OBJ是Wavefront科技开发的一种几何体图形文件格式。该格式最初是为动画工具Advanced Visualizer开发，现已开放，很多其它三维图形软件中都有使用。

![alt text](../assets/images/101_image-30.webp)
...
- 三个控制点的例子（quadratic Bezier，二阶贝塞尔曲线）：
  ![alt text](../assets/images/101_image-31.webp)

- 四个控制点的例子（cubic Bezier, 立方贝塞尔曲线））:
  ![alt text](../assets/images/101_image-32.webp)

- quadratic Bezier 曲线的插值公式:
  ![alt text](../assets/images/101_image-33.webp)
...
- 用Bernstein Polynomials(伯恩斯坦多项式)表示Bezier曲线:
  ![alt text](../assets/images/101_image-34.webp)
...
![alt text](../assets/images/101_image-35.webp)

  - 由于高阶贝塞尔曲线，比如n=10，由于每个点都贡献权重，牵一发而动全身
  - 分段贝塞尔曲线完美解决了这个问题
  - 对于分段立方贝塞尔曲线的连接处，如果切线法向在一个方向上，那么称为C0 Continuity(连续性)，大小也相同，称呼为C1连续。
    ![alt text](../assets/images/101_image-36.webp)

- Spline Curves(样条曲线) 
  可控制的曲线，没细说。

- 贝塞尔曲面
  ![alt text](../assets/images/101_image-37.webp)
  - 原理简化版：类比二次线性插值，任意t时，各个垂直方向的点作为该方向上的控制点。
    ![alt text](../assets/images/101_image-38.webp) 

### Mesh Operations: Geometry Processing 

https://sites.cs.ucsb.edu/~lingqi/teaching/resources/GAMES101_Lecture_12.pdf

![alt text](../assets/images/101_image-40.webp)

补充了一下，图灵奖获得者Pat Hanrahan，是闫令琪教授的师爷。

#### Mesh Regularization 正规化

![alt text](../assets/images/101_image-41.webp)
...
![alt text](../assets/images/101_image-54.webp)
...
![alt text](../assets/images/101_image-56.webp)
...
![alt text](../assets/images/101_image-57.webp)
...
![alt text](../assets/images/101_image-58.webp)
...
![alt text](../assets/images/101_image-59.webp)
...
- 轴对齐包围盒的好处是，不用算点乘。
  ![alt text](../assets/images/101_image-60.webp){width=500px height=500px}
- 轴对齐包围盒的面是无限大的。
- “光线进入盒子仅当它进入所有的 slabs” 是一个 前提条件,排除了平行的可能。
  ![alt text](../assets/images/101_image-61.webp)
- 那么，假定盒体上的面是“真面”，延长部分是“假面”。光线进入阶段，如果存在多次碰撞，那只有时间最大的那个才是真正触碰到真面。
- 而光线离开阶段，如果存在多次碰撞，那么时间最小的那个就是离开真面。  
  ![alt text](../assets/images/101_image-62.webp)
...
![ ](../assets/images/101_image-63.webp)
...
![alt text](../assets/images/101_image-64.webp)
...
![alt text](../assets/images/101_image-68.webp)
...
![alt text](../assets/images/101_image-69.webp)
...
#### **辐射强度（Radiant Intensity, $I$）**
  ...
  ![alt text](../assets/images/101_image-70.webp)
...
![alt text](../assets/images/101_image-71.webp)
