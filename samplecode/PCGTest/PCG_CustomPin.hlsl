...
uint Custom_GetNumData();
uint Custom_GetNumElements();
uint Custom_GetNumElements(uint DataIndex);

// Valid types: bool, int, float, float2, float3, float4, Rotator (float3), Quat (float4), Transform (float4x4), StringKey (int), Name (uint2)

<type> Custom_Get<type>(uint DataIndex, uint ElementIndex, int AttributeId);
<type> Custom_Get<type>(uint DataIndex, uint ElementIndex, 'AttributeName');
